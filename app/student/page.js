...
.pnav-ifa{width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#7c6fff,#9b8dff);display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-size:14px;font-weight:800;color:#fff;}
...
.notif-bell{position:relative;width:38px;height:38px;border-radius:10px;background:rgba(255,255,255,0.05);border:1px solid var(--border2);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;transition:background .2s;}
...
.notif-dropdown{position:absolute;top:50px;right:0;background:var(--bg2);border:1px solid var(--border2);border-radius:14px;width:300px;z-index:200;overflow:hidden;}
...
.student-avatar{width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,#7c6fff,#fb7aac);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;}
.student-chip-name{font-size:13px;font-weight:500;color:var(--text);}
.logout-btn{background:rgba(255,255,255,0.05);border:1px solid var(--border2);color:var(--text2);padding:8px 18px;border-radius:8px;font-size:12px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:background .2s;}
...
.welcome-card{background:linear-gradient(135deg,#1c1652 0%,#271e7a 45%,#1c2e5e 100%);border:1px solid rgba(155,141,255,0.2);border-radius:18px;padding:24px 28px;display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;position:relative;overflow:hidden;}
...
.tabs-row{display:flex;gap:4px;background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:5px;margin-bottom:22px;overflow-x:auto;}
.tab-btn{flex:1;min-width:90px;padding:9px 14px;border-radius:10px;border:none;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:500;cursor:pointer;white-space:nowrap;transition:all .18s;color:var(--text3);background:transparent;}
...
@media(max-width:640px){
  ...
  .tab-btn{font-size:11px;padding:8px 10px;min-width:75px;}
}
      ...
      <div className="pcontent">
        ...
        <div className="welcome-card">
          <div style={{position:"relative",zIndex:1}}>
            ...
            <div className="welcome-meta">
              ...
              <span>📊 Level: {currentLevel.code} — {currentLevel.name}</span>
            </div>
          </div>
          ...
          <div className="stats-grid">
            {[
              ...
              <div key={i} className={`stat-card ${c.cls}`}>
                ...
                <div className="stat-val" style={c.lbl==="Certification"?{fontSize:"16px",paddingTop:"6px"}:{}}>{c.val}</div>
        ...
        {tab==="progress"&&(
          <div className="section">
            ...
            <div style={{background:"linear-gradient(135deg,rgba(155,141,255,0.1),rgba(45,212,191,0.08))",border:"1px solid rgba(155,141,255,0.2)",borderRadius:14,padding:"20px 24px",marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
                <div style={{width:52,height:52,borderRadius:12,background:"rgba(155,141,255,0.15)",border:"1px solid rgba(155,141,255,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Sora',sans-serif",fontSize:18,fontWeight:800,color:"var(--purple)"}}>
                  {currentLevel.code}
                </div>
                <div>
                  <div style={{fontFamily:"'Sora',sans-serif",fontSize:18,fontWeight:700}}>{currentLevel.name}</div>
                  <div style={{fontSize:12,color:"var(--text3)"}}>{currentLevel.desc}</div>
                </div>
                <span style={{marginLeft:"auto",background:"rgba(155,141,255,0.12)",color:"var(--purple)",border:"1px solid rgba(155,141,255,0.2)",padding:"6px 14px",borderRadius:999,fontSize:12,fontWeight:700}}>Current Level</span>
              </div>
            ...
            {LEVELS.map((l,i)=>{
              ...
              return(
                <div key={i} className="journey-step">
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontWeight:600,fontSize:14,color:done?"var(--green)":curr?"var(--text)":"var(--text3)"}}>{l.name} ({l.code})</div>
                  ...
                  <span style={{fontSize:11,fontWeight:600,color:done?"var(--green)":curr?"var(--purple)":"var(--text3)"}}>{done?"✅ Done":curr?"🔥 In Progress":"🔒 Locked"}</span>
                </div>
              );
            })}
          </div>
              ...
              <>
                ...
                {data.payments.map((p,i)=>(
                  <div key={i} className="tbl-row" style={{gridTemplateColumns:"1fr 1fr 100px 80px 90px"}}>
                    ...
                    <button onClick={()=>printReceipt(p)} style={{background:"rgba(74,222,128,0.1)",color:"var(--green)",border:"1px solid rgba(74,222,128,0.2)",padding:"5px 10px",borderRadius:6,fontSize:11,cursor:"pointer",fontWeight:600}}>🖨️ Print</button>
                  </div>
                ))}
              </>
            )}
              ...
              <>
                ...
                {data.certifications.map((c,i)=>(
                  <div key={i} className="tbl-row" style={{gridTemplateColumns:"1fr 80px 100px 80px"}}>
                    ...
                    <span className="pill" style={{background:c.passed?"rgba(74,222,128,.12)":"rgba(251,122,172,.12)",color:c.passed?"var(--green)":"var(--rose)"}}>{c.passed?"✓ Pass":"✕ Fail"}</span>
                  </div>
                ))}
              </>
            )}
          ...
          <div className="section">
            ...
            {RESOURCES.map((r,i)=>(
              <div key={i} className="resource-item">
                ...
                <div style={{flex:1}}><div className="resource-name">{r.name}</div><div className="resource-desc">{r.desc}</div></div>
            ...
            <div className="section">
              ...
              <div style={{display:"flex",gap:10,marginTop:8}}>
                {!profileEdit?<button className="save-btn" onClick={()=>setProfileEdit(true)}>✏️ Edit Profile</button>:(
                  <>
                    <button className="save-btn" onClick={saveProfile} disabled={profileSaving}>{profileSaving?"Saving…":"💾 Save Changes"}</button>
