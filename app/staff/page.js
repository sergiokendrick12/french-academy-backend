...
:root{
  ...
  --g-purple:linear-gradient(135deg,#2a1f5e,#3d2b8a);
  ...
  --g-rose:linear-gradient(135deg,#3d1028,#621840);
}
...
.att-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-bottom:12px;}
...
export default function StaffPortal(){
  ...
  const login=async()=>{
    ...
    try{
      ...
      if(d.success){
        setStaff(d.staff);
        setProfilePhone(d.staff.phone||"");
        fetchData(d.staff._id);
  ...
  const submitAttendance=async()=>{
    ...
    if(Object.keys(attStatuses).length===0){toast("Please mark at least one student","error");return;}
    setAttSubmitting(true);setAttSuccess("");
    try{
      let count=0;
      for(const [studentId,status] of Object.entries(attStatuses)){
        await fetch("/api/staff/data",{method:"PUT",headers:{"Content-Type":"application/json"},
          body:JSON.stringify({studentId,className:attClass,date:attDate,status,markedBy:staff.name})});
        count++;
      }
      setAttSuccess(`✅ Attendance saved for ${count} student(s)!`);
      toast(`Attendance saved for ${count} students!`);
      setAttStatuses({});
      fetchData(staff._id);
  ...
  const myAttendance = allAttendance.filter(a=>a.markedBy===staff.name);
  const enrolledStudents = allStudents.filter(s=>s.status==="enrolled");
  const presentCount = myAttendance.filter(a=>a.status==="present").length;
  const classStudents = attClass
    ? allStudents.filter(s=>s.status==="enrolled")
    : [];
    ...
    <div style={{background:"var(--bg)",minHeight:"100vh"}}>
      ...
      <nav className="snav">
        <div className="snav-logo">
          ...
          <div>
            <div className="snav-name">International French Academy</div>
            <div className="snav-sub">Staff Portal</div>
          </div>
        </div>
      ...
      <div className="scontent">
          ...
          <>
            <div className="stats-grid">
              {[
                ...
                {cls:"green",ico:"✅",lbl:"Sessions Marked",val:myAttendance.length,    sub:"Total attendance records"},
                {cls:"amber",ico:"🎯",lbl:"Present Rate", val:myAttendance.length>0?`${Math.round((presentCount/myAttendance.length)*100)}%`:"—",sub:"Student presence"},
              ].map((c,i)=>(
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <div className="section">
                ...
                {[
                  {lbl:"Full Name",   val:staff.name},
                  {lbl:"Role",        val:staff.role},
                  {lbl:"Email",       val:staff.email},
                  {lbl:"Phone",       val:staff.phone||"—"},
                  {lbl:"Classes",     val:staff.classes},
                  {lbl:"Status",      val:staff.status},
                ].map((r,i)=>(
              ...
              <div className="section">
                ...
                <div className="section-sub">Your schedule for today</div>
                {mySchedules.length===0?(
                  ...
                  <div key={i} className="sched-item" style={{marginBottom:8}}>
                    <div>
                      ...
                      <div className="sched-name" style={{fontSize:13}}>{s.name}</div>
                      <div className="sched-meta">🕐 {s.time} · 📍 {s.room}</div>
                    </div>
            ...
            {mySchedules.length===0?(
              ...
              <div key={i} className="sched-item">
                <div style={{flex:1}}>
                  ...
                  <div className="sched-meta">🕐 {s.time} · 📊 {s.level} · 📍 {s.room}</div>
                </div>
                <span className="pill" style={{background:"rgba(96,165,250,.12)",color:"var(--blue)",flexShrink:0}}>{s.level}</span>
              </div>
            ))}
        ...
        {tab==="attendance"&&(
          <div className="section">
            ...
            <div className="att-form">
              ...
              {attClass&&(
                <>
                  <div className="profile-lbl" style={{marginBottom:10}}>
                    Mark Students ({enrolledStudents.length} enrolled)
                  </div>
                      ...
                      {enrolledStudents.map((s,i)=>(
                        <div key={i} className="att-student-row">
                          <div style={{display:"flex",alignItems:"center",gap:8,flex:1}}>
                            <div className="student-ava" style={{width:28,height:28,fontSize:10}}>
            ...
            {enrolledStudents.length===0?(
              ...
              <div key={i} className="student-item">
                ...
                <div style={{flex:1}}>
                  ...
                  <div className="student-meta">🎯 {s.certificationGoal} · 📧 {s.email}</div>
                </div>
        ...
        {tab==="history"&&(
          <div className="section">
            ...
            <div className="section-sub">Records you have marked</div>
            {myAttendance.length===0?(
              <div className="empty-state"><div className="empty-ico">📋</div><p>No attendance records yet</p></div>
            ):(
              <>
                <div className="tbl-head" style={{gridTemplateColumns:"110px 1fr 1fr 80px"}}>
                  <span>Date</span><span>Student</span><span>Class</span><span>Status</span>
                </div>
                {myAttendance.map((a,i)=>{
                  const student=allStudents.find(s=>s._id===a.studentId);
                  return(
                    <div key={i} className="tbl-row" style={{gridTemplateColumns:"110px 1fr 1fr 80px"}}>
                      <span style={{fontSize:11,color:"var(--text3)"}}>{a.date}</span>
                      <span style={{fontWeight:500,fontSize:12}}>{student?`${student.firstName} ${student.lastName}`:"Unknown"}</span>
                      <span style={{fontSize:12,color:"var(--text2)"}}>{a.className}</span>
        ...
        {tab==="profile"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div className="section">
              ...
              <input className="profile-in" value={profilePhone} disabled={!profileEdit}
                placeholder="+250 ..." onChange={e=>setProfilePhone(e.target.value)}/>
              ...
              <div style={{display:"flex",gap:10,marginTop:8}}>
                {!profileEdit?
                  <button className="save-btn" onClick={()=>setProfileEdit(true)}>✏️ Edit Profile</button>:(
                  <>
            ...
            <div className="section">
              ...
              {[
                ...
                {lbl:"Attendance Records Marked", val:myAttendance.length},
                {lbl:"Students Present", val:presentCount},
                {lbl:"Students Absent",  val:myAttendance.filter(a=>a.status==="absent").length},
                {lbl:"Students Late",    val:myAttendance.filter(a=>a.status==="late").length},
                {lbl:"Present Rate",     val:myAttendance.length>0?`${Math.round((presentCount/myAttendance.length)*100)}%`:"—"},
              ].map((r,i)=>(
