...
# Add quizResults state after quizzes state
old1 = 'const [quizzes,setQuizzes]=useState([]);'
new1 = 'const [quizzes,setQuizzes]=useState([]);const [quizResults,setQuizResults]=useState([]);'
c = c.replace(old1, new1, 1)

# Fetch quiz results on load - find the quiz fetch useEffect
old2 = "fetch('/api/admin/quiz').then(r=>r.json()).then(d=>setQuizzes(d.quizzes||[])).catch(()=>{})"
new2 = "fetch('/api/admin/quiz').then(r=>r.json()).then(d=>setQuizzes(d.quizzes||[])).catch(()=>{});fetch('/api/admin/quiz/results').then(r=>r.json()).then(d=>setQuizResults(d.results||[])).catch(()=>{})"
c = c.replace(old2, new2, 1)

# Add results table after the quiz list closing </div>
old3 = '''{showCreate&&(
      <div className="modal-bg" onClick={()=>setShowCreate(false)}>'''
new3 = '''<div style={{marginTop:32}}>
...
f.close()
print('Done!')
print('quizResults state:', c.count('quizResults'))
print('Student Results heading:', c.count('Student Results'))
