...
.stat-card.purple{background:var(--g-purple);}.stat-card.teal{background:var(--g-teal);}.stat-card.rose{background:var(--g-rose);}.stat-card.blue{background:var(--g-blue);}.stat-card.amber{background:var(--g-amber);}.stat-card.green{background:var(--g-green);}
...
.stat-card.purple .stat-val{color:var(--purple);}.stat-card.teal .stat-val{color:var(--teal);}.stat-card.rose .stat-val{color:var(--rose);}.stat-card.blue .stat-val{color:var(--blue);}.stat-card.amber .stat-val{color:var(--amber);}.stat-card.green .stat-val{color:var(--green);}
...
.quiz-item{background:var(--bg3);border:1px solid var(--border);border-radius:14px;padding:18px 20px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;}
.quiz-title{font-family:'Sora',sans-serif;font-size:15px;font-weight:700;margin-bottom:4px;}
.quiz-meta{font-size:11px;color:var(--text3);}
.quiz-start-btn{background:linear-gradient(135deg,#7c6fff,#9b8dff);color:#fff;border:none;padding:10px 22px;border-radius:10px;font-family:'Sora',sans-serif;font-size:13px;font-weight:700;cursor:pointer;white-space:nowrap;}
.quiz-done-badge{background:rgba(74,222,128,0.12);color:var(--green);border:1px solid rgba(74,222,128,0.25);padding:8px 16px;border-radius:10px;font-size:12px;font-weight:700;}
.quiz-active-wrap{background:var(--bg2);border:1px solid var(--border);border-radius:16px;padding:24px;margin-bottom:16px;}
.quiz-timer{font-family:'Sora',sans-serif;font-size:28px;font-weight:800;color:var(--rose);background:rgba(251,122,172,0.1);border:1px solid rgba(251,122,172,0.2);padding:8px 20px;border-radius:12px;}
.quiz-q-block{background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:18px;margin-bottom:14px;}
.quiz-q-text{font-size:14px;font-weight:600;margin-bottom:14px;line-height:1.6;}
.quiz-option{display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:8px;border:1px solid var(--border);cursor:pointer;margin-bottom:8px;font-size:13px;color:var(--text2);transition:all .15s;}
.quiz-option:hover{border-color:rgba(155,141,255,0.4);background:rgba(155,141,255,0.06);}
.quiz-option.selected{border-color:rgba(155,141,255,0.5);background:rgba(155,141,255,0.12);color:var(--text);}
.quiz-option.correct{border-color:rgba(74,222,128,0.5);background:rgba(74,222,128,0.1);color:var(--green);}
.quiz-option.wrong{border-color:rgba(251,122,172,0.4);background:rgba(251,122,172,0.07);color:var(--rose);}
.quiz-result-card{background:var(--bg2);border:1px solid var(--border);border-radius:16px;padding:28px;text-align:center;margin-bottom:16px;}
.quiz-score-big{font-family:'Sora',sans-serif;font-size:56px;font-weight:800;margin:12px 0;}
.quiz-submit-btn{background:linear-gradient(135deg,#7c6fff,#9b8dff);color:#fff;border:none;padding:13px 32px;border-radius:10px;font-family:'Sora',sans-serif;font-size:14px;font-weight:700;cursor:pointer;margin-top:16px;}
.quiz-submit-btn:disabled{opacity:0.5;cursor:not-allowed;}
.courses-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;margin-bottom:20px;}
.course-card{background:var(--bg2);border:1px solid var(--border);border-radius:16px;overflow:hidden;transition:transform .2s,border-color .2s;cursor:pointer;}
.course-card:hover{transform:translateY(-3px);border-color:rgba(155,141,255,0.3);}
.course-thumb{height:100px;display:flex;align-items:center;justify-content:center;font-size:36px;position:relative;}
.course-body{padding:16px;}
.course-title{font-family:'Sora',sans-serif;font-size:14px;font-weight:700;color:var(--text);margin-bottom:6px;}
.course-desc{font-size:12px;color:var(--text3);margin-bottom:12px;line-height:1.5;}
.course-progress-bar{height:5px;background:rgba(255,255,255,0.07);border-radius:999px;overflow:hidden;margin-top:10px;}
.course-progress-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#7c6fff,#9b8dff);transition:width .5s ease;}
.lesson-row{display:flex;align-items:center;gap:12px;background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:14px 16px;cursor:pointer;transition:border-color .18s;margin-bottom:8px;}
.lesson-row:hover{border-color:rgba(155,141,255,0.3);}
.lesson-row.done{border-color:rgba(74,222,128,0.2);background:rgba(74,222,128,0.04);}
.back-link{display:inline-flex;align-items:center;gap:6px;font-size:13px;color:var(--purple);cursor:pointer;margin-bottom:16px;padding:6px 0;}
@media(max-width:900px){.stats-grid{grid-template-columns:repeat(2,1fr);}.bottom-grid{grid-template-columns:1fr;}.pcontent{padding:20px 20px 80px;}}
@media(max-width:640px){
  .pnav{padding:0 12px;height:56px;}.pnav-sub{display:none;}.pnav-name{font-size:12px;}.student-chip-name{display:none;}.student-chip{padding:6px;}.logout-btn{display:none;}
  .bottom-nav{display:block;}.tabs-row{display:none;}.pcontent{padding:14px 12px 76px;}
  .welcome-card{flex-direction:column;align-items:flex-start;padding:18px;gap:12px;}.welcome-hi{font-size:20px;}
  .stats-grid{grid-template-columns:repeat(2,1fr);gap:10px;}.stat-card{padding:14px;}.stat-val{font-size:20px;}
  .section{padding:16px 14px;}.bottom-grid{grid-template-columns:1fr;gap:10px;}
  .tbl-head{display:none;}.tbl-row{padding:10px 12px;}
  .mini-stats{gap:8px;}.mini-stat{padding:10px 14px;flex:1;min-width:calc(50% - 4px);}.mini-stat-val{font-size:18px;}
...
const STATUS_MAP={new:{color:"#60a5fa",bg:"rgba(96,165,250,.12)",border:"rgba(96,165,250,.25)",label:"New"},contacted:{color:"#fbbf24",bg:"rgba(251,191,36,.12)",border:"rgba(251,191,36,.25)",label:"Contacted"},enrolled:{color:"#4ade80",bg:"rgba(74,222,128,.12)",border:"rgba(74,222,128,.25)",label:"Enrolled"},cancelled:{color:"#fb7aac",bg:"rgba(251,122,172,.12)",border:"rgba(251,122,172,.25)",label:"Cancelled"}};
const PAY_MAP={paid:{color:"#4ade80",bg:"rgba(74,222,128,.12)",label:"Paid"},pending:{color:"#fbbf24",bg:"rgba(251,191,36,.12)",label:"Pending"},partial:{color:"#9b8dff",bg:"rgba(155,141,255,.12)",label:"Partial"},waived:{color:"#94a3b8",bg:"rgba(148,163,184,.10)",label:"Waived"}};
const ATT_MAP={present:{color:"#4ade80",bg:"rgba(74,222,128,.12)",label:"Present"},absent:{color:"#fb7aac",bg:"rgba(251,122,172,.12)",label:"Absent"},late:{color:"#fbbf24",bg:"rgba(251,191,36,.12)",label:"Late"}};
const ANN_STYLES={info:{color:"#60a5fa",bg:"rgba(96,165,250,.08)",border:"rgba(96,165,250,.25)",ico:"ℹ️"},warning:{color:"#fbbf24",bg:"rgba(251,191,36,.08)",border:"rgba(251,191,36,.25)",ico:"⚠️"},success:{color:"#4ade80",bg:"rgba(74,222,128,.08)",border:"rgba(74,222,128,.25)",ico:"✅"},urgent:{color:"#fb7aac",bg:"rgba(251,122,172,.08)",border:"rgba(251,122,172,.25)",ico:"🚨"}};
const LEVELS=[{code:"A1",name:"Débutant",desc:"Complete beginner",color:"#4ade80"},{code:"A2",name:"Élémentaire",desc:"Basic communication",color:"#2dd4bf"},{code:"B1",name:"Intermédiaire",desc:"Everyday situations",color:"#60a5fa"},{code:"B2",name:"Avancé",desc:"Complex topics",color:"#9b8dff"},{code:"C1",name:"Autonome",desc:"Fluent expression",color:"#fbbf24"},{code:"C2",name:"Maîtrise",desc:"Near-native mastery",color:"#fb7aac"}];
const RESOURCES=[{ico:"📄",name:"French Alphabet Guide",desc:"PDF · Beginner",color:"rgba(96,165,250,.15)"},{ico:"🎧",name:"Listening Practice Pack",desc:"Audio ZIP · A1-A2",color:"rgba(74,222,128,.15)"},{ico:"📝",name:"TCF Québec Sample Tests",desc:"PDF · B1-B2",color:"rgba(155,141,255,.15)"},{ico:"🎬",name:"French Conversation Videos",desc:"Links · All levels",color:"rgba(251,191,36,.15)"},{ico:"📚",name:"Grammar Reference Book",desc:"PDF · All levels",color:"rgba(251,122,172,.15)"},{ico:"🗣️",name:"Pronunciation Guide",desc:"PDF + Audio · A1",color:"rgba(45,212,191,.15)"}];
const BOTTOM_NAV_TABS=[{id:"overview",ico:"🏠",lbl:"Home"},{id:"quiz",ico:"📝",lbl:"Quiz"},{id:"courses",ico:"🎓",lbl:"Courses"},{id:"payments",ico:"💳",lbl:"Pay"},{id:"profile",ico:"👤",lbl:"Me"}];
const ALL_TABS=[{id:"overview",label:"🏠 Overview"},{id:"courses",label:"🎓 Courses"},{id:"quiz",label:"📝 Quiz"},{id:"progress",label:"📈 Progress"},{id:"schedule",label:"📅 Schedule"},{id:"payments",label:"💳 Payments"},{id:"certifications",label:"🏆 Certs"},{id:"attendance",label:"📋 Attendance"},{id:"resources",label:"📚 Resources"},{id:"profile",label:"👤 Profile"}];
...
function QuizTab({student}){
  ...
  const fetchQuizzes=async()=>{
    setLoading(true);
    try{const r=await fetch(`/api/student/quiz?email=${encodeURIComponent(student.email)}`);const d=await r.json();if(d.success)setQuizzes(d.quizzes);}catch{}
    setLoading(false);
  };

  const startQuiz=async(quiz)=>{
    try{
      const r=await fetch("/api/student/quiz",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"start",quizId:quiz._id,email:student.email})});
      const d=await r.json();
      if(d.error==="already_attempted"){fetchQuizzes();return;}
      if(d.success){
        setActiveQuiz(d.quiz);setQuestions(d.quiz.questions);setAnswers({});
        setTimeLeft(d.quiz.timeLimit*60);setStartTime(Date.now());setResult(null);setGradedAnswers([]);
        timerRef.current=setInterval(()=>{
          setTimeLeft(t=>{
            if(t<=1){clearInterval(timerRef.current);submitQuiz(d.quiz,{},Date.now());return 0;}
            return t-1;
          });
        },1000);
      }
  ...
  const submitQuiz=async(quiz,ans,start)=>{
    clearInterval(timerRef.current);
    setSubmitting(true);
    const usedQuiz=quiz||activeQuiz;
    const usedAnswers=ans||answers;
    const timeTaken=start?Math.round((Date.now()-start)/1000):Math.round((Date.now()-startTime)/1000);
    try{
      const answerArray=usedQuiz.questions.map((_,i)=>usedAnswers[i]||"");
      const r=await fetch("/api/student/quiz",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"submit",quizId:usedQuiz._id,email:student.email,studentName:`${student.firstName} ${student.lastName}`,answers:answerArray,timeTaken})});
      const d=await r.json();
      if(d.success){setResult(d.result);setGradedAnswers(d.gradedAnswers);setActiveQuiz(null);fetchQuizzes();}
    }catch{}
    setSubmitting(false);
  };
  ...
  if(loading)return<div style={{textAlign:"center",padding:40,color:"var(--text3)"}}>Loading quizzes…</div>;

  // Show result
  if(result)return(
    <div>
      <div className="quiz-result-card">
        <div style={{fontSize:32,marginBottom:8}}>{result.percentage>=60?"🎉":"📚"}</div>
        <div style={{fontFamily:"'Sora',sans-serif",fontSize:18,fontWeight:700,marginBottom:4}}>{result.percentage>=60?"Congratulations!":"Keep Studying!"}</div>
        <div className="quiz-score-big" style={{color:result.percentage>=60?"var(--green)":"var(--rose)"}}>{result.percentage}%</div>
        <div style={{fontSize:13,color:"var(--text3)",marginBottom:8}}>{result.score} / {result.totalPoints} points · {Math.round(result.timeTaken/60)}m {result.timeTaken%60}s</div>
        <button className="quiz-start-btn" onClick={()=>{setResult(null);setGradedAnswers([]);}}>← Back to Quizzes</button>
      </div>
      <div className="section">
        <div className="section-title">📋 Answer Review</div>
        <div className="section-sub">See your answers vs correct answers</div>
        {gradedAnswers.map((a,i)=>(
          <div key={i} className="quiz-q-block">
            <div className="quiz-q-text">Q{i+1}: {a.question}</div>
            {a.type==="written"?(
              <div>
                <div style={{fontSize:12,color:"var(--text3)",marginBottom:4}}>Your answer:</div>
                <div style={{background:"rgba(255,255,255,0.05)",padding:"10px 14px",borderRadius:8,fontSize:13,color:"var(--text2)"}}>{a.studentAnswer||"No answer"}</div>
                <div style={{fontSize:11,color:"var(--amber)",marginTop:6}}>✏️ Written answers are reviewed manually by admin</div>
              </div>
            ):(
              <div>
                <div className={`quiz-option${a.isCorrect?" correct":" wrong"}`}>
                  {a.isCorrect?"✅":"❌"} Your answer: {a.studentAnswer||"No answer"}
                </div>
                {!a.isCorrect&&<div className="quiz-option correct">✅ Correct: {a.correctAnswer}</div>}
              </div>
            )}
          </div>
        ))}
  ...
  if(activeQuiz)return(
    <div className="quiz-active-wrap">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div>
          <div style={{fontFamily:"'Sora',sans-serif",fontSize:18,fontWeight:700}}>{activeQuiz.title}</div>
          <div style={{fontSize:12,color:"var(--text3)"}}>{questions.length} questions · {activeQuiz.level}</div>
        </div>
        <div className="quiz-timer" style={{color:timeLeft<60?"var(--rose)":"var(--green)"}}>{fmt(timeLeft)}</div>
      </div>
      {questions.map((q,qi)=>(
        <div key={qi} className="quiz-q-block">
          <div className="quiz-q-text">Q{qi+1}: {q.question}</div>
          {q.type==="multiple"&&q.options.map((opt,oi)=>(
            <div key={oi} className={`quiz-option${answers[qi]===opt?" selected":""}`} onClick={()=>setAnswers(a=>({...a,[qi]:opt}))}>
              <span style={{width:18,height:18,border:"2px solid currentColor",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10}}>{answers[qi]===opt?"●":""}</span>
              {String.fromCharCode(65+oi)}. {opt}
            </div>
          ))}
          {q.type==="truefalse"&&["True","False"].map((opt)=>(
            <div key={opt} className={`quiz-option${answers[qi]===opt?" selected":""}`} onClick={()=>setAnswers(a=>({...a,[qi]:opt}))}>
              <span style={{width:18,height:18,border:"2px solid currentColor",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10}}>{answers[qi]===opt?"●":""}</span>
              {opt}
            </div>
          ))}
          {q.type==="written"&&(
            <textarea value={answers[qi]||""} onChange={e=>setAnswers(a=>({...a,[qi]:e.target.value}))} placeholder="Write your answer here…" style={{width:"100%",padding:"11px 14px",background:"rgba(255,255,255,0.05)",border:"1px solid var(--border2)",borderRadius:10,color:"var(--text)",fontSize:14,outline:"none",minHeight:80,resize:"vertical",fontFamily:"'DM Sans',sans-serif"}}/>
          )}
        </div>
      ))}
      <div style={{display:"flex",gap:12,justifyContent:"flex-end",marginTop:8}}>
        <button onClick={()=>{clearInterval(timerRef.current);setActiveQuiz(null);}} style={{background:"rgba(255,255,255,0.05)",border:"1px solid var(--border2)",color:"var(--text2)",padding:"12px 24px",borderRadius:10,cursor:"pointer",fontSize:13}}>✕ Cancel</button>
        <button className="quiz-submit-btn" onClick={()=>submitQuiz()} disabled={submitting}>{submitting?"Submitting…":"✅ Submit Quiz"}</button>
      </div>
    </div>
  );
  ...
  return(
    <div className="section">
      ...
      {quizzes.length===0?<div className="empty-state"><div className="empty-ico">📝</div><p>No quizzes available yet</p></div>
      :quizzes.map((q,i)=>(
        <div key={i} className="quiz-item">
          <div style={{flex:1}}>
            <div className="quiz-title">{q.title}</div>
            <div className="quiz-meta">⏱ {q.timeLimit} min · 📊 {q.level} · ❓ {q.questionCount} questions</div>
            {q.attempted&&q.result&&<div style={{marginTop:6,fontSize:12,color:q.result.percentage>=60?"var(--green)":"var(--rose)",fontWeight:600}}>Last score: {q.result.percentage}% {q.result.percentage>=60?"✅":"❌"}</div>}
          </div>
          {q.attempted
            ?<div className="quiz-done-badge">✅ Completed · {q.result?.percentage}%</div>
            :<button className="quiz-start-btn" onClick={()=>startQuiz(q)}>Start Quiz →</button>
          }
        </div>
      ))}
    </div>
...
export default function StudentPortal(){
  ...
  const login=async()=>{
    ...
    try{const r=await fetch("/api/student/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email})});const d=await r.json();if(d.success){setStudent(d.student);setProfileForm({firstName:d.student.firstName,lastName:d.student.lastName,email:d.student.email,phone:d.student.phone||""});fetchData(d.student._id);fetchAnnouncements();}else setErr(d.error||"Student not found.");}catch{setErr("Connection error.");}
    finally{setLoading(false);}
  };

  const fetchData=async(id)=>{try{const r=await fetch("/api/student/data",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({studentId:id})});const d=await r.json();if(d.success)setData(d);}catch{}};
  const fetchAnnouncements=async()=>{try{const r=await fetch("/api/admin/announcements");const d=await r.json();setAnnouncements((d.announcements||[]).filter(a=>a.active&&(a.audience==="all"||a.audience==="students")));}catch{}};
  const logout=()=>{setStudent(null);setData(null);setEmail("");setTab("overview");};

  const saveProfile=async()=>{
    setProfileSaving(true);setProfileMsg("");
    try{const r=await fetch("/api/student/data",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({studentId:student._id,...profileForm})});const d=await r.json();if(d.success){setStudent(s=>({...s,...profileForm}));setProfileMsg("✅ Profile updated!");setProfileEdit(false);}else setProfileMsg("❌ Failed to update.");}catch{setProfileMsg("❌ Connection error.");}
    finally{setProfileSaving(false);}
  };

  const printReceipt=(p)=>{const w=window.open("","_blank");w.document.write(`<html><head><title>Receipt</title></head><body><h2>IFA Receipt</h2><p>Amount: ${p.amount} RWF</p><p>Method: ${p.method}</p><p>Date: ${p.date}</p><p>Status: ${p.status}</p><script>window.print();</script></body></html>`);w.document.close();};
  ...
  if(!student)return(
    ...
    <div className="login-wrap">
      ...
      <div className="login-card">
        ...
        <div className="login-flag"><div className="flag-bar" style={{width:18,background:"#002395"}}/><div className="flag-bar" style={{width:18,background:"#fff"}}/><div className="flag-bar" style={{width:18,background:"#ED2939"}}/><span>France · Rwanda</span></div>
      ...
      <nav className="pnav">
        ...
        <div className="pnav-right">
          <div style={{position:"relative"}} ref={notifRef}>
            <div className="notif-bell" onClick={()=>setShowNotif(v=>!v)}>🔔{unreadNotifs>0&&<span className="notif-badge">{unreadNotifs}</span>}</div>
            {showNotif&&(<div className="notif-dropdown"><div className="notif-header"><span>Notifications</span>{unreadNotifs>0&&<span style={{fontSize:11,color:"var(--purple)",cursor:"pointer"}} onClick={()=>setReadNotifs(announcements.map(a=>a._id))}>Mark all read</span>}</div>{announcements.length===0?<div style={{padding:"20px",textAlign:"center",color:"var(--text3)",fontSize:12}}>No notifications</div>:announcements.map((a,i)=>{const isRead=readNotifs.includes(a._id);const c=ANN_STYLES[a.type]||ANN_STYLES.info;return(<div key={i} className={`notif-item${isRead?"":" notif-unread"}`} onClick={()=>setReadNotifs(r=>[...new Set([...r,a._id])])}><div className="notif-title" style={{color:c.color}}>{c.ico} {a.title}</div><div className="notif-msg">{a.message}</div></div>);})}</div>)}
          </div>
          <div className="student-chip" onClick={()=>setTab("profile")}><div className="student-avatar">{initials(student.firstName,student.lastName)}</div><span className="student-chip-name">{student.firstName} {student.lastName}</span></div>
      ...
      <div className="pcontent">
        ...
        <div className="welcome-card">
          <div style={{position:"relative",zIndex:1,flex:1}}>
            ...
            <div className="welcome-meta"><span>🎯 {student.certificationGoal||"—"}</span><span>📅 {fmtDate(student.createdAt)}</span><span>📊 {currentLevel.code} — {currentLevel.name}</span></div>
          </div>
          <span className="enrolled-pill" style={{background:ss.bg,color:ss.color,borderColor:ss.border}}><span className="e-dot" style={{background:ss.color}}/>{ss.label}</span>
        </div>

        <div className="tabs-row">{ALL_TABS.map(t=><button key={t.id} className={`tab-btn${tab===t.id?" active":""}`} onClick={()=>setTab(t.id)}>{t.label}</button>)}</div>

        {tab==="overview"&&(<>
          <div className="stats-grid">
            {[{cls:"purple",ico:"💰",lbl:"Total Paid",val:`${totalPaid.toLocaleString()}`,sub:"RWF · Confirmed"},{cls:"teal",ico:"⏳",lbl:"Pending",val:`${totalPending.toLocaleString()}`,sub:"RWF · Outstanding"},{cls:"rose",ico:"🏆",lbl:"Pass Rate",val:passRate!==null?`${passRate}%`:"—",sub:`${certCount} exam(s)`},{cls:"blue",ico:"📋",lbl:"Attendance",val:attendanceRate!==null?`${attendanceRate}%`:"—",sub:`${attTotal} sessions`},{cls:"amber",ico:"🎯",lbl:"Goal",val:student.certificationGoal||"—",sub:"Certification"},{cls:"green",ico:"📚",lbl:"Classes",val:data?.schedules?.length||"—",sub:"Available"}].map((c,i)=>(
              <div key={i} className={`stat-card ${c.cls}`}><div className="stat-ico">{c.ico}</div><div className="stat-lbl">{c.lbl}</div><div className="stat-val" style={c.lbl==="Goal"?{fontSize:"15px",paddingTop:"6px"}:{}}>{c.val}</div><div className="stat-sub">{c.sub}</div></div>
            ))}
          </div>
          <div className="bottom-grid">
            <div className="msg-card"><div className="section-title" style={{marginBottom:10}}>💬 Your Message</div><p className="msg-text">{student.message?`"${student.message}"`:<span style={{opacity:0.4}}>No message yet.</span>}</p></div>
            <div className="help-card"><div className="section-title">📞 Need Help?</div><div className="section-sub">Contact the academy directly</div><div className="help-btns"><a href="https://wa.me/250785302957" target="_blank" rel="noopener noreferrer" style={{textDecoration:"none"}}><button className="btn-wa">💬 WhatsApp</button></a><a href="mailto:frenchacademyinternational@gmail.com" style={{textDecoration:"none"}}><button className="btn-email">📧 Send Email</button></a></div></div>
          </div>
        </>)}

        {tab==="quiz"&&<QuizTab student={student}/>}

        {tab==="courses"&&(
          <div className="section">
            <div className="section-title">🎓 My Courses</div>
            <div className="section-sub">Your assigned online courses</div>
            {!data?.courses?.length?<div className="empty-state"><div className="empty-ico">🎓</div><p>No courses assigned yet</p></div>
            :data.courses.map((c,i)=>(
              <div key={i} className="resource-item">
                <div className="resource-ico" style={{background:"rgba(155,141,255,0.15)"}}>📄</div>
                <div style={{flex:1,minWidth:0}}><div className="resource-name">{c.title}</div><div className="resource-desc">{c.description} · {c.level}</div></div>
                {c.fileUrl&&<a href={c.fileUrl} target="_blank" rel="noopener noreferrer"><button className="dl-btn">⬇️ Télécharger</button></a>}
              </div>
            ))}
          </div>
        )}
          ...
          <div className="section">
            ...
            <div style={{background:"linear-gradient(135deg,rgba(155,141,255,0.1),rgba(45,212,191,0.08))",border:"1px solid rgba(155,141,255,0.2)",borderRadius:14,padding:"20px",marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14,flexWrap:"wrap"}}>
                ...
                <div style={{flex:1}}><div style={{fontFamily:"'Sora',sans-serif",fontSize:17,fontWeight:700}}>{currentLevel.name}</div><div style={{fontSize:12,color:"var(--text3)"}}>{currentLevel.desc}</div></div>
                <span style={{background:"rgba(155,141,255,0.12)",color:"var(--purple)",border:"1px solid rgba(155,141,255,0.2)",padding:"6px 14px",borderRadius:999,fontSize:12,fontWeight:700}}>Current Level</span>
              </div>
            ...
            {LEVELS.map((l,i)=>{const done=i<currentLevelIdx,curr=i===currentLevelIdx;return(
              <div key={i} className="journey-step">
                <div className={`step-circle ${done?"step-done":curr?"step-current":"step-future"}`}>{done?"✓":l.code}</div>
                <div style={{flex:1}}><div style={{fontWeight:600,fontSize:13,color:done?"var(--green)":curr?"var(--text)":"var(--text3)"}}>{l.name} ({l.code})</div><div style={{fontSize:11,color:"var(--text3)"}}>{l.desc}</div>{curr&&<div style={{marginTop:6}}><div className="progress-bar-wrap" style={{maxWidth:200}}><div className="progress-bar-fill" style={{width:`${levelProgress}%`,background:l.color}}/></div><div style={{fontSize:10,color:"var(--text3)",marginTop:3}}>{levelProgress}% complete</div></div>}</div>
          ...
          <div className="section">
            ...
            :data.schedules.map((c,i)=>(
              <div key={i} className="sched-item">
                <div style={{flex:1,minWidth:0}}><div className="sched-day">{c.day}</div><div className="sched-name">{c.name}</div><div className="sched-meta">🕐 {c.time} · {c.level} · 👨‍🏫 {c.teacher} · 📍 {c.room}</div></div>
            ...
            {!data?.payments?.length?<div className="empty-state"><div className="empty-ico">💳</div><p>No payment records yet</p></div>:(
              <>
                ...
                {data.payments.map((p,i)=>(
                  <div key={i} className="tbl-row" style={{gridTemplateColumns:"1fr 1fr 100px 80px 90px"}}>
                    <span style={{fontWeight:700,color:"var(--green)"}}>{p.amount.toLocaleString()} RWF</span>
                    ...
                    <button onClick={()=>printReceipt(p)} style={{background:"rgba(74,222,128,0.1)",color:"var(--green)",border:"1px solid rgba(74,222,128,0.2)",padding:"6px 10px",borderRadius:6,fontSize:11,cursor:"pointer"}}>🖨️ Print</button>
                  </div>
                ))}
              </>
            )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div className="section">
              ...
              <div style={{textAlign:"center",marginBottom:24}}>
                ...
                <div style={{fontSize:12,color:"var(--text3)",marginTop:4}}>{student.email}</div>
              ...
              {["firstName","lastName","email","phone"].map(f=>(
                <div key={f} style={{marginBottom:16}}>
                  ...
                  <input className="profile-in" value={profileForm[f]||""} disabled={!profileEdit} onChange={e=>setProfileForm(p=>({...p,[f]:e.target.value}))}/>
                </div>
              ))}
              <div style={{display:"flex",gap:10,marginTop:8}}>
                {!profileEdit?<button className="save-btn" onClick={()=>setProfileEdit(true)}>✏️ Edit Profile</button>:(
                  <><button className="save-btn" onClick={saveProfile} disabled={profileSaving}>{profileSaving?"Saving…":"💾 Save"}</button><button onClick={()=>{setProfileEdit(false);setProfileMsg("");}} style={{background:"rgba(255,255,255,0.05)",border:"1px solid var(--border2)",color:"var(--text2)",padding:"12px 20px",borderRadius:10,cursor:"pointer",fontSize:13}}>Cancel</button></>
                )}
              </div>
            </div>
            <div className="section">
              ...
              {[{lbl:"Certification Goal",val:student.certificationGoal||"—"},{lbl:"Enrolled Date",val:fmtDate(student.createdAt)},{lbl:"Student Status",val:ss.label},{lbl:"Classes Available",val:data?.schedules?.length||"—"},{lbl:"Total Paid",val:`${totalPaid.toLocaleString()} RWF`},{lbl:"Attendance Rate",val:attendanceRate!==null?`${attendanceRate}%`:"—"},{lbl:"Current Level",val:`${currentLevel.code} — ${currentLevel.name}`}].map((r,i)=>(
              ...
              <div style={{marginTop:20}}><div style={{fontSize:13,fontWeight:700,marginBottom:12}}>📞 Contact Academy</div><div className="help-btns"><a href="https://wa.me/250785302957" target="_blank" rel="noopener noreferrer" style={{textDecoration:"none"}}><button className="btn-wa" style={{fontSize:12,padding:"8px 16px"}}>💬 WhatsApp</button></a><a href="mailto:frenchacademyinternational@gmail.com" style={{textDecoration:"none"}}><button className="btn-email" style={{fontSize:12,padding:"8px 16px"}}>📧 Email</button></a></div></div>
              <button onClick={logout} style={{marginTop:20,width:"100%",padding:"12px",background:"rgba(251,122,172,0.08)",border:"1px solid rgba(251,122,172,0.2)",color:"var(--rose)",borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer"}}>🚪 Sign Out</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="bottom-nav-inner">
          {BOTTOM_NAV_TABS.map(t=>(
            <button key={t.id} className={`bnav-btn${tab===t.id?" active":""}`} onClick={()=>setTab(t.id)}>
              <span className="bnav-ico">{t.ico}</span><span className="bnav-lbl">{t.lbl}</span><span className="bnav-dot"/>
            </button>
          ))}
          <button className={`bnav-btn${!BOTTOM_NAV_TABS.find(t=>t.id===tab)?" active":""}`} onClick={()=>setTab("progress")}>
            <span className="bnav-ico">•••</span><span className="bnav-lbl">More</span><span className="bnav-dot"/>
          </button>
        </div>
      </div>
    </div></>
